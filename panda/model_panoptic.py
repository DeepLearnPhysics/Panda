...
class MaskQueryDecoder(nn.Module):
    ...
    def _max_seqlen(self, seq_len: int) -> int:
        self.__max_seqlen = max(seq_len, self.__max_seqlen)
    ...
    def _get_queries(self, point: Point) -> Tuple[torch.Tensor, Optional[torch.Tensor], torch.Tensor, torch.Tensor]:
        ...
        # superpoint queries
        assert "instance_dbscan" in point
        ...
        for b in range(batch_size):
            ...
            if inst_mask.any():
                ...
                unique_labels, inv = torch.unique(
                    inst_labels, return_inverse=True, sorted=True
                )
                inst_queries = inst_feats.new_zeros(
                    (unique_labels.numel(), inst_feats.shape[-1])
                ).index_add(0, inv, inst_feats)
                counts = torch.bincount(inv, minlength=unique_labels.numel()).to(
                    inst_feats.dtype
                )
                inst_queries = inst_queries / counts.unsqueeze(1)
...
class Detector(PointModule):
    ...
    def postprocess(
        self,
        forward_output: dict,
        stuff_threshold: float | None = None,
        mask_threshold: float | None = None,
        conf_threshold: float | None = None,
        nms_kernel: str | None = None,
        nms_sigma: float | None = None,
        nms_pre: int | None = None,
        nms_max: int | None = None,
        min_points: int | None = None,
        background_class_label: int | None = None,
        fill_uncovered: bool | None = None,
    ):
