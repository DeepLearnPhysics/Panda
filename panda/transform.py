...
class ConditionalRandomTransform(object):
    ...
    def __call__(self, data_dict):
        ...
        for dim, axis in enumerate(("x", "y", "z")):
            ...
            if (min_val >= bounds[0] + self.buffer_size) and (
                max_val <= bounds[1] - self.buffer_size
            ):
                continue
...
class MultiViewGenerator(object):
    ...
    def get_view(self, point, center, scale, size_override: Optional[int] = None):
        ...
        size = (
            int(np.random.uniform(*scale) * max_size)
    ...
    def get_center(self, coord, mask=None):
        ...
        if self.center_sampling == "cnms":
            ...
            possible_centers, _, _ = cnms(
                possible_centers, **self.center_sampling_kwargs
            )
    ...
    def __call__(self, data_dict):
        ...
        anchors_pool = (
            np.concatenate(anchors_pool, axis=0)
            if len(anchors_pool) > 0
        ...
        size_base = int(
            np.mean(
                [
                    np.random.uniform(*self.local_view_scale)
                    * min(self.max_size, coord.shape[0])
                    for _ in range(4)
                ]
            )
        )
        size_override = int(
            max(8, min(self.max_size, size_base * (self.anchor_radius_scale**3)))
        )

        # Determine counts
        num_anchor_locals = (
            int(np.ceil(self.local_view_num * float(self.anchor_bias_ratio)))
...
class InstanceParser(object):
    ...
    def __call__(self, data_dict):
        ...
        centroid = (
            np.ones((coord.shape[0], 3), dtype=coord_dtype) * self.instance_ignore_index
        )
        bbox = (
            np.ones((instance_num, 8), dtype=coord_dtype) * self.instance_ignore_index
        )
        vacancy = [
        ...
        for instance_id in range(instance_num):
            ...
            if self.compute_axis_stats:
                ...
                if point_count >= self.axis_min_points:
                    centered = coord_.astype(
                        np.float32, copy=False
                    ) - bbox_centroid.astype(np.float32, copy=False)
                    if np.linalg.norm(centered, axis=1).max() > self.axis_eps:
                        ...
                        principal_vec = eigvecs[:, principal_index].astype(
                            np.float32, copy=False
                        )
                        principal_norm = float(np.linalg.norm(principal_vec))
                        if (
                            principal_norm > self.axis_eps
                            and principal_val > self.axis_eps
                        ):
                            ...
                            if axis_length_value > self.axis_eps:
                                valid_axis = True
                                axis_coord_local = projections.astype(
                                    coord_dtype, copy=False
                                )
                                denom = (
                                    axis_length_value * 0.5
                                    if self.axis_normalize_half_extent
                                    else axis_length_value
                                )
                                denom = float(denom) + self.axis_eps
                                axis_coord_norm_local = (
                                    axis_coord_local / denom
                                ).astype(coord_dtype, copy=False)
                if not valid_axis:
                    axis_vec = axis_default
...
class LocalCovarianceFeatures(object):
    ...
    def __call__(self, data_dict):
        ...
        if self.gaussian_weight:
            ...
            mean = (
                np.sum(offsets * w[..., None], axis=1, keepdims=True) / w_sum[..., None]
            )
            centered = offsets - mean
            cov = (
                np.einsum("nki,nkj->nij", centered * w[..., None], centered)
                / w_sum[..., None]
            )
        else:
        ...
        suml = (l1 + l2 + l3) + 1e-12
        curvature = l3 / suml
        local_shape = np.stack([r21, r32, r31, curvature], axis=1).astype(
            coord.dtype, copy=False
        )
