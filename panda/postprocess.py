...
def postprocess_panoptic(
    ...
    if stuff_classes:
        stuff_tensor = torch.tensor(list(stuff_classes), dtype=pred_labels.dtype, device=device)
    ...
    # initialize outputs
    pred_instance_labels = torch.full((num_points,), -1, dtype=torch.long, device=device)
    pred_class_labels = torch.full((num_points,), background_class_label, dtype=torch.long, device=device)
    ...
    if keep.any():
        ...
        # apply matrix NMS
        nms_scores, nms_labels, nms_masks, _ = mask_matrix_nms(
            sel_masks, sel_labels, combined_scores,
            ...
            if uncovered.any():
                ...
                for q in order.tolist():
                    if pred_labels[keep][q].item() == num_classes:
                        continue
                    if stuff_classes and int(pred_labels[keep][q].item()) in stuff_classes:
                        continue
                    mask = sel_masks[q]
                    if not mask.any():
                        continue
    ...
    **kwargs,
    ...
    if stuff_probs is not None and point_counts is not None:
        point_offsets = torch.cat([
            torch.zeros(1, dtype=torch.long, device=point_counts.device),
            point_counts.cumsum(dim=0),
        ])
    else:
        point_offsets = None
